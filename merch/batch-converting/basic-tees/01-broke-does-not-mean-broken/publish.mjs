...
const uploaded = await printify("/uploads/images.json", {
  method: "POST",
  body: JSON.stringify({
    file_name: "broke-not-broken-tee.png",
    contents,
  }),
});
...
const handle = "https://brokedadsclub.com/shop/broke-not-broken-tee";
try {
...
const result = {
  ...
  firstMockup: images.find((img) => img.position === "front" && img.src?.includes("camera_label=front"))?.src
    ?? images[0]?.src
    ?? null,
};
