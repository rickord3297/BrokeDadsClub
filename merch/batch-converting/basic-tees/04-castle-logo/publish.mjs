...
const uploaded = await printify("/uploads/images.json", {
  method: "POST",
  body: JSON.stringify({
    file_name: "castle-crest-tee.png",
    contents,
  }),
});
...
if (!chosen.length) throw new Error("No matching shirt variants.");
...
const product = await printify(`/shops/${shopId}/products.json`, {
  method: "POST",
  body: JSON.stringify({
    title: "Castle Crest Tee",
    description:
      "The official Broke Dads Club crest. Castle you cannot actually afford, printed on soft Gildan cotton after you check out. For dads who know the ticket price.",
    tags: ["broke dads club", "castle crest", "dad tee"],
...
const handle = "https://brokedadsclub.com/shop/castle-crest-tee";
try {
...
for (let attempt = 1; attempt <= 10 && images.length < 4; attempt++) {
...
const result = {
  ...
  firstMockup:
    images.find((img) => img.position === "front" && img.src?.includes("camera_label=front"))
